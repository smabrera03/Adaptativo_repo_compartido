...
3.a) Si el ángulo comandado es mayor que el ángulo máximo permitido --> empezar a bajar la barra
3.b) Si el ángulo comandado es menor que el ángulo mínimo permitido --> volver a subir la barra
*/
...
#define INCREMENTO 2.0 //pasos de 2° de incremento. Elegí este valor en base a la resolución del servo, que es de 1,5° aprox. 
...
void loop() {
  ...
  if(angulo > ANGULO_SERVO_MAX){
    delta_ang = -INCREMENTO;
    angulo += 2*delta_ang;
  } else if(angulo < ANGULO_SERVO_MIN){
    delta_ang = INCREMENTO;
    angulo += 2*delta_ang;
  }
